...
// It adds a 50% chance that the person will hit instead of stay if their hand is less than 17
public class ChanceStrategy implements Strategy{
    private final Map<String, String> table = populateTable();

    // get the move from the hash table
    public String getMove(Hand player, Hand dealer) {
        ...
        if((player.getCurrentHandValue() < 17) && (move == "s")){
            if(Math.random() <= .5){
                move = "h";
            }
        }
    ...
    // combine the user hand and dealer hand to form the key for the move table
